...
export function ModelSettingsModal({
  ...
  const fetchOllamaModels = async (silent = false) => {
    ...
    if (trimmedEndpoint && !validateOllamaEndpoint(trimmedEndpoint)) {
      const errorMsg = 'URL do endpoint do Ollama inválida. Deve começar com http:// ou https://';
      setError(errorMsg);
      if (!silent) {
    ...
    try {
      ...
      const errorMsg = err instanceof Error ? err.message : 'Falha ao carregar modelos do Ollama';
      setError(errorMsg);
  ...
  const loadBuiltinAiModels = async () => {
    ...
    try {
      ...
      toast.error('Falha ao carregar modelos Built-in AI');
    }
  };
  ...
  const handleSave = async () => {
    ...
    if (modelConfig.provider === 'custom-openai') {
      try {
        ...
        toast.error('Falha ao salvar configuração personalizada do OpenAI');
        return;
      }
    }
  ...
  const testCustomOpenAIConnection = async () => {
    if (!customOpenAIEndpoint.trim() || !customOpenAIModel.trim()) {
      toast.error('Digite primeiro a URL do endpoint e o nome do modelo');
      return;
    }
    ...
    try {
      ...
      toast.success(result.message || 'Conexão realizada com sucesso!');
    } catch (err) {
  ...
  const downloadRecommendedModel = async () => {
    ...
    if (isDownloading(recommendedModel)) {
      toast.info(`${recommendedModel} já está sendo baixado`, {
        description: `Progresso: ${Math.round(getProgress(recommendedModel) || 0)}%`
      });
      return;
    }
    ...
    try {
      ...
      const errorMsg = err instanceof Error ? err.message : 'Falha ao baixar modelo';
      console.error('Error downloading model:', err);

      // Check if Ollama is not installed and show appropriate error
      if (isOllamaNotInstalledError(errorMsg)) {
        toast.error('O Ollama não está instalado', {
          description: 'Baixe e instale o Ollama antes de baixar modelos.',
          duration: 7000,
          action: {
            label: 'Baixar',
  ...
  const deleteOllamaModel = async (modelName: string) => {
    try {
      ...
      const errorMsg = err instanceof Error ? err.message : 'Falha ao excluir modelo';
      toast.error(errorMsg);
          ...
          <div className="flex space-x-2 mt-1">
            ...
            >
              ...
              <SelectContent className="max-h-64 overflow-y-auto">
                <SelectItem value="builtin-ai">Built-in AI (Offline, Sem API necessária)</SelectItem>
                ...
                <PopoverTrigger asChild>
                  ...
                  >
                    <span className="truncate">
                      {modelConfig.model || "Selecione o modelo..."}
                    </span>
        ...
        {modelConfig.provider === 'custom-openai' && (
          <div className="space-y-4 border-t pt-4">
            ...
            <div>
              ...
              />
              <p className="text-xs text-muted-foreground mt-1">
                Identificador do modelo para as requisições
              </p>
            </div>
            ...
            <div>
              ...
              <Input
                ...
                placeholder="Deixe em branco se não for necessária"
                className="mt-1"
              />
            </div>
              ...
              <div className="absolute inset-y-0 right-0 pr-1 flex items-center space-x-1">
                {apiKey?.trim() && (
                  <Button
                    ...
                    title={isApiKeyLocked ? 'Desbloquear para editar' : 'Bloquear para evitar edição'}
                  >
            ...
            {!isEndpointSectionCollapsed && (
              <>
                <p className="text-sm text-muted-foreground mt-1 mb-2">
                  Deixe em branco ou digite um endpoint personalizado (ex.: http://x.yy.zz:11434)
                </p>
                ...
                {ollamaEndpointChanged && !error && (
                  <Alert className="mt-3 border-yellow-500 bg-yellow-50">
                    <AlertDescription className="text-yellow-800">
                      Endpoint alterado. Clique em "Buscar Modelos" para carregar os modelos do novo endpoint antes de salvar.
                    </AlertDescription>
                  </Alert>
                )}
              </>
        ...
        {modelConfig.provider === 'ollama' && (
          <div>
            <div className="flex items-center justify-between mb-4">
              <h4 className="text-sm font-bold">Modelos Ollama Disponíveis</h4>
            ...
            {isLoadingOllama ? (
                  ...
                  <div className="space-y-4">
                    <Alert className="border-orange-500 bg-orange-50">
                      <AlertDescription className="text-orange-800">
                        O Ollama não está instalado ou não está em execução. Baixe e instale o Ollama para utilizar modelos locais.
                      </AlertDescription>
                    </Alert>
                    <Button
                      variant="default"
                    ...
                    <div className="text-sm text-muted-foreground text-center">
                      Após instalar o Ollama, reinicie este aplicativo e clique em "Buscar Modelos" para continuar.
                    </div>
                  </div>
                ) : (
                    ...
                    <Alert className="mb-4">
                      <AlertDescription>
                        {ollamaEndpointChanged
                          ? 'Endpoint alterado. Clique em "Buscar Modelos" para carregar modelos do novo endpoint.'
                          : 'Nenhum modelo encontrado. Baixe um modelo recomendado ou clique em "Buscar Modelos" para carregar modelos disponíveis do Ollama.'}
                      </AlertDescription>
                    </Alert>
                    {!ollamaEndpointChanged && (
                      <div className="space-y-3">
                        ...
                        >
                          {isDownloading('gemma3:1b') ? (
                            ...
                            <>
                              ...
                              Baixar gemma3:1b (Recomendado, ~800MB)
                            </>
                          )}
                        </Button>
              ...
              <ScrollArea className="max-h-[calc(100vh-450px)] overflow-y-auto pr-4">
                {filteredModels.length === 0 ? (
                  <Alert>
                    <AlertDescription>
                      Nenhum modelo encontrado para "{searchQuery}". Tente outro termo de busca.
                    </AlertDescription>
                  </Alert>
                ) : (
