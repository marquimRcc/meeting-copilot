...
export function SummaryModelSettings({ refetchTrigger }: SummaryModelSettingsProps) {
  ...
  const handleSaveModelConfig = async (config: ModelConfig) => {
    try {
      ...
      toast.success('Configurações do modelo salvas com sucesso');
    } catch (error) {
      console.error('Error saving model config:', error);
      toast.error('Falha ao salvar configurações do modelo');
    }
  };
  ...
  return (
    <div className='flex flex-col gap-4'>
      <div className="bg-white rounded-lg border border-gray-200 p-6 shadow-sm">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-600">Gerar resumo automaticamente após finalizar a reunião</p>
          </div>
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-6 shadow-sm">
        <h3 className="text-lg font-semibold mb-4">Configuração do Modelo de Resumo</h3>
        <p className="text-sm text-gray-600 mb-6">
          Configure o modelo de IA utilizado para gerar resumos de reunião.
        </p>
