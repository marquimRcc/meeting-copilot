                        ...
                        <div className="flex items-center gap-1.5 mb-1">
                            <span
                                className={`text-[11px] font-medium px-1.5 py-0.5 rounded ${
                                    source === 'Microphone' || speaker === 'Você'
                                        ? 'bg-blue-50 text-blue-700 border border-blue-200'
                                        : 'bg-purple-50 text-purple-700 border border-purple-200'
                                }`}
                            >
                                {speaker || (source === 'Microphone' ? 'Você' : 'Participante')}
                            </span>
                        </div>
                    )}
                    {isStreaming ? (
                    ...
                    {isRecording ? (
                        <>
                            ...
                            <p className="text-sm text-gray-600">
                                {isPaused ? 'Gravação pausada' : 'Aguardando fala...'}
                            </p>
                            <p className="text-xs mt-1 text-gray-400">
                                {isPaused ? 'Clique em retomar para continuar gravando' : 'Fale para ver a transcrição ao vivo'}
                            </p>
                        </>
                    ) : (
                        <>
                            <p className="text-lg font-semibold">Bem-vindo ao Meetily!</p>
                            <p className="text-xs mt-1">Inicie a gravação para ver a transcrição ao vivo</p>
                        </>
                    )}
                </motion.div>
            ) : useVirtualization ? (
                        ...
                        <div ref={loadMoreTriggerRef} className="flex justify-center items-center py-4 mt-2">
                            {isLoadingMore ? (
                                ...
                                <span className="text-sm text-gray-400">
                                    Exibindo {loadedCount} de {totalCount} trechos
                                </span>
                            ) : null}
                        </div>
                    )}
