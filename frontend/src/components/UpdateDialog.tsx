...
export function UpdateDialog({ open, onOpenChange, updateInfo }: UpdateDialogProps) {
    ...
    if (open && updateInfo?.available) {
      ...
      check().then((updateResult) => {
        if (updateResult?.available) {
          ...
          setError('Atualização não está mais disponível');
        }
      }).catch((err) => {
        console.error('Failed to get update object:', err);
        setError('Falha ao preparar atualização: ' + (err.message || 'Erro desconhecido'));
      });
    } else {
  ...
  const handleDownloadAndInstall = async () => {
    ...
    if (!updateToUse) {
      try {
        ...
        if (updateResult?.available) {
          ...
          setError('Atualização indisponível');
          return;
        }
      } catch (err: any) {
        setError('Falha ao obter atualização: ' + (err.message || 'Erro desconhecido'));
        return;
      }
    }
    ...
    try {
      ...
      setError(err.message || 'Falha ao baixar ou instalar atualização');
      setIsDownloading(false);
      toast.error('Falha na atualização: ' + (err.message || 'Erro desconhecido'));
    }
  };
