...
export function AudioBackendSelector({
  ...
  useEffect(() => {
    const loadBackends = async () => {
      try {
        ...
        setError('Falha ao carregar opções de backend');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleBackendChange = async (backendId: string) => {
    try {
      ...
      setError('Falha ao alterar backend. Por favor, tente novamente.');
    }
  };
  ...
  return (
    <div className="space-y-2">
        ...
        <div className="relative">
          ...
          {showTooltip && (
            <div className="absolute z-10 left-6 top-0 w-64 p-3 text-xs bg-gray-900 text-white rounded-lg shadow-lg">
              <p className="font-semibold mb-1">Métodos de Captura de Áudio:</p>
              ...
              <p className="mt-2 text-gray-300">
                Experimente diferentes backends para encontrar o que melhor funciona no seu sistema.
              </p>
            </div>
          )}
        </div>
      ...
      <div className="text-xs text-gray-500 space-y-1">
        <p>• A seleção de backend afeta apenas a captura de áudio do sistema</p>
        <p>• O microfone sempre utiliza o método padrão</p>
        <p>• As alterações se aplicam a novas sessões de gravação</p>
      </div>
    </div>
  );
}